import {
  ...
  getCommuteTimesForLGA,
  getLatestReliabilityWeek,
  getReliabilityForLGA,
  getNSWInfrastructureForLGA,
...
const sqliteLiveDataRepository: LiveDataRepository = {
  ...
  getNSWProjection,
  getTransportStaticForLGA,
  getCommuteTimesForLGA(lgaCode) {
    return getCommuteTimesForLGA(lgaCode).map((r) => ({
      lga_code: r.lga_code,
  ...
  getLatestReliabilityWeek(lgaCode) {
    return getLatestReliabilityWeek(lgaCode);
  },
  getReliabilityForLGA(lgaCode, weekStart) {
    return getReliabilityForLGA(lgaCode, weekStart).map((r) => ({
      lga_code: r.lga_code,
      mode: r.mode,
      hour_of_day: r.hour_of_day,
      week_start: r.week_start,
      pct_on_time: r.pct_on_time,
      trip_count: r.trip_count,
    }) satisfies GtfsReliabilityRecord);
  },
  getNSWInfrastructureForLGA(lgaCode) {
    return getNSWInfrastructureForLGA(lgaCode).map((r) => ({
      lga_code: r.lga_code,
