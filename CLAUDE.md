...
- No `preferRealData` toggle; all pages show live data where available, sample data otherwise
- `DataSourceBadge` component (`@/components/ui/DataSourceBadge`) shows live vs. sample status

## Data Sources
| Source | Coverage | Update |
|---|---|---|
| ABS Census API (SDMX-ML XML) | Population, medians, dwelling type, tenure, industry, education, SEIFA, labour | `npm run seed:abs` |
...
## Admin Panel
- URL: `/admin` (no auth in dev; protected by `ADMIN_KEY` env var in prod)
- Shows ABS cache count, projection count, config values, recent refresh logs
- POST `/api/admin/refresh` with `{ mode: 'static' | 'abs' | 'all', lgaId? }` to trigger seed
- Same logic as CLI seed script, runs in-process via `src/lib/seed-runner.ts`
...
| `tfnsw_cache` | `(lga_code, dataset)` | Reserved for future live TfNSW API calls |
